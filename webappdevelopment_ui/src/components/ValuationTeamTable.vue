        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Valuation Teams</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalValuationTeams = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalValuationTeams">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add ValuationTeam</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="ValuationTeamId" type="text" placeholder="Enter ValuationTeamId" v-model="valuationTeamToAdd.valuationTeamId"></base-input>
  <base-input label="TeamName" type="text" placeholder="Enter TeamName" v-model="valuationTeamToAdd.teamName"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="valuationTeams" :row-key="record => record.ValuationTeamId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <ValuationTeamPictureView :valuationTeams="valuationTeams" />
          </div>
...
import ValuationTeamPictureView from './ValuationTeamPictureView.vue';


const valuationTeamsColumns = [
  "valuationTeamId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    ValuationTeamPictureView
  },

  data() {
    return {
      modalValuationTeams: false,
        isTableView: true,

      columns: [
        {
          title: 'Valuation Team Id',
		dataIndex: 'valuationTeamId',
          visible: true,
          scopedSlots: { customRender: 'valuationTeamId' },
          sorter: true
          //sorter: (a, b) => a.valuationTeamId - b.valuationTeamId,
          //sorter: (a, b) => a.valuationTeamId.localeCompare(b.valuationTeamId),
        },
        {
          title: 'Team Name',
		dataIndex: 'teamName',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} valuationTeams`,
      },
      ...
      valuationTeamsTable: {
        columns: [...valuationTeamsColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'valuationTeamId',           // Column to sort by
  ...
  methods: {
    ...
    async renderValuationTeamsTable() {
      ...
      let valuationTeamsTableData = [];
      for (let i = 0; i < this.valuationTeams.length; i++) {
        valuationTeamsTableData.push({
          id: i,
          valuationTeamId: this.valuationTeams[i].valuationTeamId,
          year: this.valuationTeams[i].year,
          date: this.valuationTeams[i].date,
          competitionId: this.valuationTeams[i].competitionId,
          valuationTeamId: this.valuationTeams[i].valuationTeamId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-valuation-teams',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForValuationTeamsChanged', this.searchQuery);
		//this.renderValuationTeamsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalValuationTeams = false;

      const currentDate = new Date().getTime();
      this.valuationTeamToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.valuationTeamToAdd);
      console.log(jsonData);
      
      const res = await ValuationTeamService.addValuationTeam(jsonData);
