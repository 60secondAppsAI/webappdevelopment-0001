        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Deal Reps</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalDealReps = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalDealReps">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add DealRep</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="DealRepId" type="text" placeholder="Enter DealRepId" v-model="dealRepToAdd.dealRepId"></base-input>
  <base-input label="Name" type="text" placeholder="Enter Name" v-model="dealRepToAdd.name"></base-input>
  <base-input label="Contact" type="text" placeholder="Enter Contact" v-model="dealRepToAdd.contact"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="dealReps" :row-key="record => record.DealRepId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <DealRepPictureView :dealReps="dealReps" />
          </div>
...
import DealRepPictureView from './DealRepPictureView.vue';


const dealRepsColumns = [
  "dealRepId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'dealRepId',
          visible: true,
          scopedSlots: { customRender: 'dealRepId' },
          sorter: true
          //sorter: (a, b) => a.dealRepId - b.dealRepId,
          //sorter: (a, b) => a.dealRepId.localeCompare(b.dealRepId),
        },
        {
          title: 'Name',
		dataIndex: 'name',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} dealReps`,
      },
      ...
      dealRepsTable: {
        columns: [...dealRepsColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'dealRepId',           // Column to sort by
  ...
  methods: {
    ...
    async renderDealRepsTable() {
      ...
      let dealRepsTableData = [];
      for (let i = 0; i < this.dealReps.length; i++) {
        dealRepsTableData.push({
          id: i,
          dealRepId: this.dealReps[i].dealRepId,
          year: this.dealReps[i].year,
          date: this.dealReps[i].date,
          competitionId: this.dealReps[i].competitionId,
          dealRepId: this.dealReps[i].dealRepId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-deal-reps',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForDealRepsChanged', this.searchQuery);
		//this.renderDealRepsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalDealReps = false;

      const currentDate = new Date().getTime();
      this.dealRepToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.dealRepToAdd);
      console.log(jsonData);
      
      const res = await DealRepService.addDealRep(jsonData);
