        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalWebsites = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalWebsites">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Website</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="WebsiteId" type="text" placeholder="Enter WebsiteId" v-model="websiteToAdd.websiteId"></base-input>
  <base-input label="SeoStrategy" type="text" placeholder="Enter SeoStrategy" v-model="websiteToAdd.seoStrategy"></base-input>
  <base-input label="ContentStrategy" type="text" placeholder="Enter ContentStrategy" v-model="websiteToAdd.contentStrategy"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="websites" :row-key="record => record.WebsiteId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <WebsitePictureView :websites="websites" />
          </div>
...
import WebsitePictureView from './WebsitePictureView.vue';


const websitesColumns = [
  "websiteId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'websiteId',
          visible: true,
          scopedSlots: { customRender: 'websiteId' },
          sorter: true
          //sorter: (a, b) => a.websiteId - b.websiteId,
          //sorter: (a, b) => a.websiteId.localeCompare(b.websiteId),
        },
        {
          title: 'Seo Strategy',
		dataIndex: 'seoStrategy',
          visible: true,
          scopedSlots: { customRender: 'seoStrategy' },
          sorter: true
          //sorter: (a, b) => a.seoStrategy - b.seoStrategy,
          //sorter: (a, b) => a.seoStrategy.localeCompare(b.seoStrategy),
        },
        {
          title: 'Content Strategy',
		dataIndex: 'contentStrategy',
          visible: true,
          scopedSlots: { customRender: 'contentStrategy' },
          sorter: true
          //sorter: (a, b) => a.contentStrategy - b.contentStrategy,
          //sorter: (a, b) => a.contentStrategy.localeCompare(b.contentStrategy),
        },
      ],
      pagination: {
        current: 1,
      ...
      sortBy: 'websiteId',           // Column to sort by
  ...
  methods: {
    ...
    async renderWebsitesTable() {
      ...
      let websitesTableData = [];
      for (let i = 0; i < this.websites.length; i++) {
        websitesTableData.push({
          id: i,
          websiteId: this.websites[i].websiteId,
          year: this.websites[i].year,
          date: this.websites[i].date,
          competitionId: this.websites[i].competitionId,
          websiteId: this.websites[i].websiteId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-websites',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForWebsitesChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalWebsites = false;

      const currentDate = new Date().getTime();
      this.websiteToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.websiteToAdd);
      console.log(jsonData);
      
      const res = await WebsiteService.addWebsite(jsonData);
