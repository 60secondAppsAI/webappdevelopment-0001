...
<template>
        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Shipping Boxs</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalShippingBoxs = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalShippingBoxs">
                <template slot="header">
  ...
  <base-input label="ShippingInstructions" type="text" placeholder="Enter ShippingInstructions" v-model="shippingBoxToAdd.shippingInstructions"></base-input>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="shippingBoxs" :row-key="record => record.ShippingBoxId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <ShippingBoxPictureView :shippingBoxs="shippingBoxs" />
          </div>
...
import ShippingBoxPictureView from './ShippingBoxPictureView.vue';


const shippingBoxsColumns = [
  "shippingBoxId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    ShippingBoxPictureView
  },

  data() {
    return {
      modalShippingBoxs: false,
        isTableView: true,

      columns: [
        {
          title: 'Shipping Box Id',
		dataIndex: 'shippingBoxId',
          visible: true,
          scopedSlots: { customRender: 'shippingBoxId' },
          sorter: true
          //sorter: (a, b) => a.shippingBoxId - b.shippingBoxId,
          //sorter: (a, b) => a.shippingBoxId.localeCompare(b.shippingBoxId),
        },
        {
          title: 'Box Details',
		dataIndex: 'boxDetails',
          visible: true,
          scopedSlots: { customRender: 'boxDetails' },
          sorter: true
          //sorter: (a, b) => a.boxDetails - b.boxDetails,
          //sorter: (a, b) => a.boxDetails.localeCompare(b.boxDetails),
        },
        {
          title: 'Shipping Instructions',
		dataIndex: 'shippingInstructions',
          visible: true,
          scopedSlots: { customRender: 'shippingInstructions' },
          sorter: true
          //sorter: (a, b) => a.shippingInstructions - b.shippingInstructions,
          //sorter: (a, b) => a.shippingInstructions.localeCompare(b.shippingInstructions),
        },
      ],
      pagination: {
        current: 1,
        ...
        showTotal: (total) => `Total ${total} shippingBoxs`,
      },
      ...
      shippingBoxsTable: {
        columns: [...shippingBoxsColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'shippingBoxId',           // Column to sort by
  ...
  methods: {
    ...
    async renderShippingBoxsTable() {
      ...
      let shippingBoxsTableData = [];
      for (let i = 0; i < this.shippingBoxs.length; i++) {
        shippingBoxsTableData.push({
          id: i,
          shippingBoxId: this.shippingBoxs[i].shippingBoxId,
          year: this.shippingBoxs[i].year,
          date: this.shippingBoxs[i].date,
          competitionId: this.shippingBoxs[i].competitionId,
          shippingBoxId: this.shippingBoxs[i].shippingBoxId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-shipping-boxs',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForShippingBoxsChanged', this.searchQuery);
		//this.renderShippingBoxsTable();
    },
	...
	async handleAddSubmitted() {
      ...
      this.shippingBoxToAdd.created = currentDate;
