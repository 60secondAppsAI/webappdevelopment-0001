        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Loan Repayments</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalLoanRepayments = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalLoanRepayments">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add LoanRepayment</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="LoanRepaymentId" type="text" placeholder="Enter LoanRepaymentId" v-model="loanRepaymentToAdd.loanRepaymentId"></base-input>
  <base-input label="PaidAmount" type="text" placeholder="Enter PaidAmount" v-model="loanRepaymentToAdd.paidAmount"></base-input>
  <base-input label="PaymentDate" type="text" placeholder="Enter PaymentDate" v-model="loanRepaymentToAdd.paymentDate"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="loanRepayments" :row-key="record => record.LoanRepaymentId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <LoanRepaymentPictureView :loanRepayments="loanRepayments" />
          </div>
...
import LoanRepaymentPictureView from './LoanRepaymentPictureView.vue';


const loanRepaymentsColumns = [
  "loanRepaymentId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    LoanRepaymentPictureView
  },

  data() {
    return {
      modalLoanRepayments: false,
        isTableView: true,

      columns: [
        {
          title: 'Loan Repayment Id',
		dataIndex: 'loanRepaymentId',
          visible: true,
          scopedSlots: { customRender: 'loanRepaymentId' },
          sorter: true
          //sorter: (a, b) => a.loanRepaymentId - b.loanRepaymentId,
          //sorter: (a, b) => a.loanRepaymentId.localeCompare(b.loanRepaymentId),
        },
        {
          title: 'Paid Amount',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} loanRepayments`,
      },
      ...
      sortBy: 'loanRepaymentId',           // Column to sort by
  ...
  methods: {
    ...
    async renderLoanRepaymentsTable() {
      ...
      let loanRepaymentsTableData = [];
      for (let i = 0; i < this.loanRepayments.length; i++) {
        loanRepaymentsTableData.push({
          id: i,
          loanRepaymentId: this.loanRepayments[i].loanRepaymentId,
          year: this.loanRepayments[i].year,
          date: this.loanRepayments[i].date,
          competitionId: this.loanRepayments[i].competitionId,
          loanRepaymentId: this.loanRepayments[i].loanRepaymentId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-loan-repayments',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForLoanRepaymentsChanged', this.searchQuery);
		//this.renderLoanRepaymentsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalLoanRepayments = false;

      const currentDate = new Date().getTime();
      this.loanRepaymentToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.loanRepaymentToAdd);
      console.log(jsonData);
      
      const res = await LoanRepaymentService.addLoanRepayment(jsonData);
