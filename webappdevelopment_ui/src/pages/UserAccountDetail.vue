					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateUserAccount()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2UserAccounts = true">Add</base-button>
								              <modal :show.sync="modal2UserAccounts">
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">User Account Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="UserAccountId" type="text" placeholder="Enter UserAccountId" v-model="userAccountDetails.userAccountId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Username</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="Username" type="text" placeholder="Enter Username" v-model="userAccountDetails.username"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Password</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="Password" type="text" placeholder="Enter Password" v-model="userAccountDetails.password"></input>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  methods: {
    ...
    async updateUserAccount() {
      ...
      const jsonData = JSON.stringify(this.userAccountDetails);
      const res = await UserAccountService.update(jsonData);
...
//        this.getAllUserAccounts();
    ...
    async getUserAccountDetails() {
      try {
			let response = await UserAccountService.get(this.userAccountId);
